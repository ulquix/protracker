          ...
          {total?.macros &&
            Object.keys(total.macros).map((key) => (
              ...
              >
                ...
                <div className="absolute top-0 left-0 bg-foreground text-background w-full h-full text-wrap opacity-0 select-none hover:opacity-80 active:opacity-80 rounded-lg flex flex-col justify-center items-center p-2">
                  <div className=" text-xl">daily value:</div>
                  <div>{dailyRequirements.macros[key as keyof Macros]}</div>
                </div>
              </div>
            ))}
        </div>
            ...
            Object.keys(total.micros).map((micro) => (
              ...
              >
                ...
                <div className="text-primary flex text-lg">
                  ...
                  <div className="absolute top-0 left-0 bg-foreground text-background w-full h-full text-wrap opacity-0 select-none hover:opacity-80 active:opacity-80 rounded-lg flex flex-col justify-center items-center p-2">
                    <div className=" text-xl">daily value:</div>
                    <div>{dailyRequirements.micros[micro as keyof Micros]}</div>
                  </div>
                </div>
              </div>
            ))}
